...
- provider registry의 현재 연결 상태는 `onboarding_required`다. 실제 endpoint,
  credential, 암호화 저장소와 데이터 계약은 고객·provider 계약 후 운영 환경에서
  주입한다. 이 저장소에는 실제 기업 호가 fixture를 넣지 않는다.
