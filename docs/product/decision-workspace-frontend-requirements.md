...
이 문서는 `origin/main`의 `f130a52`를 기준으로 실제 브라우저에서 KB Comme
...
이 문서는 closed 상태로 종료되고 병합되지 않은
[PR #7: 거래 Workspace와 AI 코파일럿 요구사항](https://github.com/GreenGreenAI/tradeflow/pull/7)의
후속·대체 요구사항이다. PR #7의 문서는 `main`에 들어오지 않았고 최종 리뷰
상태도 `CHANGES_REQUESTED`로 남아 있다.

Role B가 PR #7에서 요청한 변경과 이 문서의 반영 상태는 다음과 같다.

| PR #7 변경 요청 | 이 문서의 반영 |
|---|---|
| 독립 화면 6개를 MVP 필수로 두지 말 것 | 별도 URL이나 화면 수를 요구하지 않고 현재 대화 답변 안의 구조화 결과를 보완 |
| RM Console·Review Service를 제외할 것 | `review_required` 표시만 요구하며 RM 화면과 서비스는 범위 밖 |
| 인증·영속 저장을 MVP 인수 조건으로 추가하지 말 것 | 실제 인증·저장을 요구하지 않고 데모 상태를 정직하게 표시하거나 실제 분석 state와 맞출 것만 요구 |
| 브라우저 E2E를 필수 CI로 만들지 말 것 | 기존 단위·통합 테스트를 필수로 유지하고 브라우저 E2E는 optional extra |
| 현재 구현을 채팅 전용이라고 표현하지 말 것 | 자연어 입력과 구조화된 FX 분석이 구현됐음을 §3에서 명시하고, 손실되는 응답 필드만 격차로 정의 |

PR #7의 초기 제안처럼 거래목록, 독립 TradeProgram 상세, 포트폴리오 또는 Action
Queue를 이번 MVP의 필수 화면으로 다시 열지 않는다. 이 문서는 최신 `main`에서
실제로 동작하는 대화형 분석 화면을 유지하면서, 이미 실행되는 Role A 판정 결과가
프론트 경계에서 유실되지 않게 하는 좁은 보완 요구사항이다.

PR #51이 병합되면 프론트 보완 작업에서는 이 문서를 현재 기준으로 사용하고,
PR #7은 결정 배경을 확인하는 이력으로만 남긴다.
...
KB Comme의 최종 산출물은 분석 수치가 아니라 담당자가 수행할 거래별
...
단위·통합 테스트는 필수다. Playwright 등 별도 브라우저 E2E 도구 도입과 해당
테스트의 CI 필수화는 이 문서의 수용 조건이 아니며 optional extra로 둔다.
