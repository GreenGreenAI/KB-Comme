...
## 고객의 거래 맥락을 구조화하고 관련 정보를 수집해, 시나리오별 근거와 금융기관 상담 준비까지 연결하는 수출입금융 의사결정 AI
...
TradeFlow는 고객의 현재 기업·거래·자금·위험 맥락을 확인된 사실로 구조화하고, 필요한 정보를
...
같은 기준으로 비교하는 것이다. TradeFlow는 결정을 대신 내리기보다 선택지별 결과와 근거,
...
> **TradeFlow — 고객의 거래 맥락을 구조화하고 필요한 정보를 수집해, 시나리오별 판단 근거와
> 금융기관 상담 준비까지 연결하는 수출입금융 의사결정 AI**

### 3.2 한 줄 가치 제안

> 고객의 기업·거래·자금 상황을 입력하면 필요한 정보를 확인·조회하고, 자금 공백·환노출·헤지
> 대안을 시나리오별로 비교하여 의사결정 근거와 금융기관 상담자료로 남긴다.

### 3.3 기획의 차별화 축

TradeFlow는 환위험 관리를 공통 기반으로 가져간다. 다만 “환위험을 분석한다”는 주제 자체를
독점적 차별점으로 주장하지 않는다. 환위험 분석에 결제일 자금 공백을 결합하고, 그 결과가
어떻게 바뀌며 실제 상담으로 어떻게 이어지는지를 제품화한다.

```text
고객·거래 맥락 구조화               출발점
+ 사용자·시스템·전문가 정보 수집    맥락 보완
+ 환위험·결제일 자금 공백 계산      공통 기반
+ 선택지별 시나리오 비교            판단 가치
+ 결과를 바꾸는 질문과 Delta         차별화 가치
+ 근거·준비자료·상담 후속조치        실행 가치
```
...
TradeFlow의 사용자 경험은 `맥락 인식 → 정보 수집 → 시나리오 분석 → 근거 제공`의 네 단계로
...
필요한 외화를 별도로 보유하거나 조달해야 한다. TradeFlow는 전체 금액의 차이와 날짜별 외화잔액을
분리해서 계산한다.
...
### 8.7 TradeFlow가 문제를 처리하는 순서

#### 1단계: 두 거래가 모두 입력됐는지 확인한다

TradeFlow는 수입 거래와 수출 거래를 각각 방향, 통화, 금액, 결제일과 결제방식으로 구조화한다.
...
TradeFlow는 최초 조건에서 다음과 같이 표시한다.
...
현재 USD 잔액은 최대 자금 공백을 직접 줄일 수 있다. 따라서 TradeFlow는 여러 누락정보를
...
순노출은 바뀌지 않는다. TradeFlow의 Decision Delta는 실제로 변경된 값과 변경되지 않은 값을
구분한다.
...
행동을 구체화하는 정보다. 사용자가 사전상담을 했다고 기록하면 TradeFlow는 해당 사실을 사용자
기록으로 보존하지만, 이를 은행의 실제 접수·승인 정보로 표시하지 않는다.

### 8.10 상담자료 생성과 권한 범위

사용자가 동의하면 TradeFlow는 Decision Passport를 생성한다. 이 자료에는 다음 내용이 포함된다.
...
이 예시는 TradeFlow가 다음 효용을 제공한다는 것을 보여준다.
...
따라서 TradeFlow가 이 시나리오에서 제공하는 결과는 `금융실행 결과`가 아니라 `결제일별 필요한
...
| 출품 주제 | 제목에서 확인되는 중심 | TradeFlow와의 관계 |
|---|---|---|
| TradePilot FX | 결제일 기반 자연헤지와 손실한도 분석 | 환위험·결제일 분석이라는 공통 기반 |
| KB 환부장 | 수출입 중소기업 환리스크 관리 | 핵심 고객과 환위험 관리라는 공통 기반 |
| TradeFlow | 환위험·자금 공백에서 결정 변화와 상담 준비까지 연결 | 공통 기반 위에 의사결정 보완·증빙·실행 준비를 추가 |

따라서 TradeFlow의 발표 전략은 “우리는 환위험 서비스가 아니다”가 아니라 다음과 같이 정리한다.
...
TradeFlow는 고객의 현재 기업·거래·자금·위험 상황을 확인 가능한 맥락으로 구조화하는 데서
...
즉, TradeFlow는 “AI가 무엇이라고 답했는가”보다 “고객의 상황을 어떤 맥락으로 이해했고,
...
해결
TradeFlow는 고객의 현재 맥락을 구조화하고 필요한 정보를 수집한 뒤,
자금 공백·환노출·헤지 대안을 시나리오별로 계산합니다.

차별화
맥락 인식 → 정보 수집 → 시나리오 비교 → 의사결정 근거
